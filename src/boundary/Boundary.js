...
export function redrawCanvas(model, canvasObj) {
    ...
    for (let r = 0; r < nr; r++) {
        for (let c = 0; c < nc; c++) {
            ...
            if (cell.type == 'ninjase' || cell.type == 'wall' || cell.type == 'space') ctx.fillRect(sq.x, sq.y, sq.size, sq.size)
            ...
            // need to address ninja-se holding a key
            if (cell.type == 'key') ctx.fillRect(sq.x + sq.size * 0.25, sq.y + sq.size * 0.25, sq.size * 0.5, sq.size * 0.5)
